...
{  
    public class LessonUtils
    {
        ...
        public LessonUtils()
        {
            ...
            url = "http://jwgl.zqu.edu.cn/";
            request = (HttpWebRequest)WebRequest.Create("http://www.zxbmsl.top/Setting.ashx");
            request.Method = "GET";
        ...
        public int isNeedCode()
        {
            ...
            string result;
            request = (HttpWebRequest)WebRequest.Create(url + "login!isNeedVerify.action?_=" + DateTime.Now.Millisecond);
            request.Method = "GET";
            //第一次请求
            if (cookies == null)
            { 
        ...
        public Image getCodeImage()
        {
            ...
            request = (HttpWebRequest)WebRequest.Create(url + "yzm?d=" + ms);
        ...
        {
            request = (HttpWebRequest)WebRequest.Create(url + "login!doLogin.action");
            request.KeepAlive = true;
            request.Method = "POST";
            request.CookieContainer = cookies;
            //登陆参数 账号、密码、验证码
        ...
        public dynamic executeLogin(string id, string pwd, string code)
        {
            ...
            if (json["free"] == "0")
            {
                ...
                {
                    //请求获得注册情况
                    request = (HttpWebRequest)WebRequest.Create("http://www.zxbmsl.top/IsRegister.ashx?id=" + id);
                    request.Method = "GET";
                    request.KeepAlive = false;
                    response = (HttpWebResponse)request.GetResponse();
                ...
                if (result == "0")
                {
                    ...
                    {
                        Process.Start("http://zxbmsl.top/Register.aspx");
                        msg.Add("msg", "请完成激活. . .");
                    }
                    else
                    {
        ...
        public List<string[]> getCourseList()
        {
            ...
            string jsonStr = Send(url + "xsxklist!getDataList.action", "POST", "page=1&rows=200&sort=kcrwdm&order=asc", url + "xsxklist!xsmhxsxk.action");
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            dynamic json;
            try
            {
        ...
        public void Send(object p)
        {
            ...
            string json;
            HttpWebRequest request = WebRequest.Create(url + "xsxklist!getAdd.action") as HttpWebRequest;
            request.Method = "POST";
            request.KeepAlive = true;
            request.CookieContainer = cookies;
            request.ServicePoint.Expect100Continue = false;
            request.Referer = url + "xsxklist!xsmhxsxk.action";
